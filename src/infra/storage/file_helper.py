...
from src.shared.helpers.text_helper import plain_text

MIN_TEXT_LENGTH = 50
MAX_TEXT_LENGTH = 600


def csv_to_json(csv_path: str, json_path: str) -> list[dict[str, Any]]:
    ...
    clean_data[:] = [item for item in clean_data if MIN_TEXT_LENGTH < len(item.get("text", "")) <= MAX_TEXT_LENGTH]
...
def write_json(json_path: str | Path, data: dict[str, Any] | list[Any]) -> None:
    json_file_path = Path(json_path)
    ...
    print(f"Successfully saved to {json_file_path}")  # noqa: T201
