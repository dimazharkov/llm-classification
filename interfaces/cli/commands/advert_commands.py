...
from interfaces.cli.handlers.advert_handlers import advert_analyze, advert_indexing, advert_preprocess, advert_summarize
...
def preprocess(
    ...
    category_ids: list[int] = typer.Option(
        SELECTED_CATEGORIES_IDS, help="Список ID категорий, например: 11239 11243 595",
    ),
):
    advert_preprocess(source_path, target_path, per_category, list(category_ids))
...
def indexing(
    ...
    advert_indexing(source_path, target_path)
...
def analyze(
    ...
    advert_analyze(source_path, rejected_path)
